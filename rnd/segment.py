...
class SegList(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        data, name = [Image.open(join(self.data_dir, self.image_list[index]))], self.image_list[index]
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, name) in enumerate(eval_data_loader):
        data_time.update(time.time() - end)
        image_var = Variable(image[0], requires_grad=False, volatile=True)
...
def test_seg(args):
    ...
    #test_image = "/home/ananth/Desktop/dog.jpg"
    if args.ms:
...
def parse_args():
    ...
    parser.add_argument('--with-gt', default=False, type=bool)
